...
def encrypt(msg, passw):
    iv = 8 * b'\0'
    des = DES.new(passw, DES.MODE_CFB, iv)
    encd = des.encrypt(msg)
    return encd
...
for file_roi in os.listdir(roi_path):
    ...
    with open(roi_path + "/" + file_roi, "rb") as roi:
        ...
        # Allungo stringa contenente i bytes della ROI in un multiplo di 8 (per DES)
        img_decoded += ((8 - len(img_decoded) % 8) * "0")
